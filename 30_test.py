...
def main(pdf_path, output_excel_path, existing_excel_path, key_columns):
    ...
    extracted_texts = extract_highlighted_text_tesseract(pdf_path, page_number, highlight_regions, debug_mode=DEBUG_MODE)
    ...
    processed_df = match_highlighted_texts_with_table_fuzzy(extracted_texts, processed_df, threshold=80, debug_mode=DEBUG_MODE)

    # 기존 엑셀 데이터 로드
    df_existing = load_existing_excel_data(existing_excel_path, key_columns)

    if df_existing.empty:
        # 기존 엑셀 파일이 없으면, 새로운 엑셀 파일로 저장
        print(f"기존 엑셀 파일이 없으므로, 새로운 엑셀 파일을 생성합니다: {output_excel_path}")
        processed_df = processed_df.drop_duplicates(subset=key_columns)
        processed_df.to_excel(output_excel_path, index=False)
        print(f"새로운 엑셀 파일이 '{output_excel_path}'에 생성되었습니다.")
    else:
        # 새로운 데이터와 기존 데이터 비교 및 업데이트
        update_excel_data(processed_df, df_existing, key_columns, output_excel_path, debug_mode=DEBUG_MODE)

    # 최종 엑셀 파일에 강조 표시 (선택 사항)
    # save_to_excel_with_highlight(df_updated, output_excel_path, debug_mode=DEBUG_MODE)
...
if __name__ == "__main__":
    ...
    existing_excel_path = "/workspaces/automation/output/extracted_tables_camelot.xlsx"  # 기존 엑셀 파일 경로
