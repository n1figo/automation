...
def pdf_to_image(page):
    try:
        ...
        # 이미지 크기 줄이기
        img.thumbnail((1000, 1000))  # 최대 크기를 1000x1000으로 제한
        logging.info("PDF page converted to image and resized successfully")
        return np.array(img)
    except Exception as e:
...
def extract_text_from_image(image):
    try:
        ...
        sections = []
        for i in range(0, height, 500):  # 500픽셀 높이의 섹션으로 나누기
            section = image[i:i+500, :]
        ...
        logging.info("Text extracted from image using OCR (sectioned processing)")
        return results
    except Exception as e:
...
def process_ocr_result(ocr_result, highlight_regions):
    try:
        ...
        logging.error(f"Error in process_ocr_result: {str(e)}")
        raise
