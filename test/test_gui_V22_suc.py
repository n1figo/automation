...
class TableProcessor:
    ...
    def clean_table_content(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # 헤더 패턴
            exact_header_pattern = re.compile(r'^보\s+장\s+명\s+지\s+급\s+사\s+유\s+지\s+급\s+금\s+액$')
            ...
            for idx, row in df.iterrows():
                ...
                # 헤더 패턴 확인
                if exact_header_pattern.match(normalized_value):
                    rows_to_drop.append(idx)
                    continue
                
                # 무시할 패턴 확인
                should_ignore = any(re.search(pattern, cell_value) 
    ...
    def standardize_columns(self, df: pd.DataFrame, page_num: int) -> pd.DataFrame:
        ...
        try:
            ...
            if all(str(col).isdigit() for col in df.columns):
                ...
                if '보 장 명' in first_cell:
                    # 데이터 분리
                    new_data = []
                    for idx in range(1, len(df)):
                        cell_value = str(df.iloc[idx,0])
                        if cell_value:
                            row_data = cell_value.split('\n')
                            if len(row_data) >= 3:
                                new_data.append({
                                    '담보명': row_data[0].strip(),
                                    '지급사유': row_data[1].strip(),
                                    '지급금액': row_data[2].strip(),
                                    '변경사항': '',
                                    '페이지': page_num
                                })
