...
def main():
    ...
    for page in select_pages:
        ...
        if results:
            ...
            end_query = "합계"
            end_results = search_rag(end_query, page_index, model, page_chunks, k=1)
            if end_results:
                end_position = page_text.index(end_results[0]['content'])
                print(f"페이지 {page}에서 표 끝 위치: {end_position}")
            else:
                print(f"페이지 {page}에서 표 끝을 찾지 못했습니다.")
