...
def extract_tables_with_t5(html_content):
    chunks = [html_content[i:i+512] for i in range(0, len(html_content), 512)]
    
    tables = []
    for chunk in chunks:
        input_text = f"Extract table structure from HTML: {chunk}"
        input_ids = tokenizer.encode(input_text, return_tensors="pt", max_length=512, truncation=True)
        
        outputs = model.generate(input_ids, max_length=150, num_return_sequences=1)
        decoded_output = tokenizer.decode(outputs[0], skip_special_tokens=True)
        
        if "table" in decoded_output.lower():
            tables.append(decoded_output)
...
def extract_tables_from_html(html_file_path):
    try:
        ...
        extracted_tables = extract_tables_with_t5(html_content)
...
def extract_specific_table_with_t5(html_content):
    chunks = [html_content[i:i+512] for i in range(0, len(html_content), 512)]
    
    specific_table = None
    for chunk in chunks:
        input_text = f"Extract table from '상해 관련 특별약관' section: {chunk}"
        input_ids = tokenizer.encode(input_text, return_tensors="pt", max_length=512, truncation=True)
        
        outputs = model.generate(input_ids, max_length=200, num_return_sequences=1)
        decoded_output = tokenizer.decode(outputs[0], skip_special_tokens=True)
        
        if "상해 관련 특별약관" in decoded_output and "table" in decoded_output.lower():
            specific_table = decoded_output
            break
    
    if specific_table:
        rows = specific_table.split('\n')
        if len(rows) > 1:
            headers = rows[0].split('|')
            data = [row.split('|') for row in rows[1:]]
            df = pd.DataFrame(data, columns=headers)
...
def extract_specific_table(html_file_path):
    try:
        ...
        return extract_specific_table_with_t5(html_content)
    except Exception as e:
