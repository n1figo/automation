        ...
        try:
            ...
            if self.example_analysis_var.get() and example_pdf_path and not coverage_path:
                thread = threading.Thread(
                    ...
                    args=(None, None, example_pdf_path)  # coverage_path는 None로 설정
                )
                thread.start()
                return
...
# app.py의 HTML 처리 관련 메서드 수정
    ...
    def process_files(self, coverage_path, html_path, example_pdf_path):
        """파일 처리 작업 수행"""
        try:
            self.update_progress(0, "처리 시작...")
            results = {
                'web': [],    # HTML/MHTML 데이터
                'coverage': [],  # 보장내용 PDF 데이터
            }
            ...
            if mhtml_path:
                ...
                self.log_message(f"MHTML 파일: {mhtml_path}")
                try:
                    extractor = HTMLFileExtractor(mhtml_path)
            ...
            # 2. 보장내용 PDF 처리 (기존 코드 유지)
            if coverage_path and self.coverage_analysis_var.get():
                self.update_progress(40, "보장내용 PDF 분석 중...")
                self.log_message(f"보장내용 PDF 파일: {coverage_path}")
                try:
                    coverage_analyzer = PDFAnalyzer(coverage_path, self.logger)
                    output_path = coverage_analyzer.analyze()
                    if output_path:
            ...
            # 3. 결과 저장
            if results['web'] or results['coverage']:
                ...
                if results['web']:
                    ...
                    self.log_message(f"웹 테이블 저장 완료: {web_output}")
                ...
                result_files = []
                if results['web']:
                    result_files.append(str(web_output))
                if results['coverage']:
                    result_files.append(results['coverage'])
    ...
    def save_web_tables(self, tables: List[pd.DataFrame], output_path: str):
        """웹 테이블 저장"""
        wb = WorkbookManager(output_path)
        ...
        for idx, df in enumerate(tables):
            # 메타데이터 추가
            df = self._add_table_metadata(df, idx + 1)
            ...
            # 테이블 작성
            start_row = wb.write_table(sheet, df, start_row, table_info)
        ...
        if mhtml_files:
            ...
            for mhtml_file in mhtml_files:
                ...
                if tables:
                    ...
                    self.log_message(f"{mhtml_file}에서 테이블 추출에 실패했습니다.", "WARNING")
        else:
