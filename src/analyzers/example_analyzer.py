...
class ExamplePDFAnalyzer:
    ...
    def analyze(self) -> Dict:
        ...
        try:
            ...
            if tables_data:
                excel_path = self.save_to_excel(tables_data)
                ...
                return {
                    "success": True,
                    "message": "분석이 완료되었습니다. 결과 폴더가 열렸습니다.",
                    "data": {
                        "excel_path": str(excel_path),
                        "summary": self._generate_summary(tables_data)
                    }
                }
            else:
                return {
                    "success": False,
                    "message": "분석할 테이블을 찾을 수 없습니다.",
                    "data": None
                }
    ...
    def _generate_summary(self, tables_data: Dict) -> Dict:
        """분석 결과 요약 생성"""
        summary = {
            "total_tables": sum(len(tables) for tables in tables_data.values()),
            "sections": list(tables_data.keys()),
