...
class TableExtractor:
    ...
    def clean_table_content(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df = df[~df.apply(lambda row: any(
                re.match(pattern, str(value).strip()) for pattern in header_patterns for value in row
            ), axis=1)]
            ...
            for _, row in df.iterrows():
                ...
                for pattern in self.ignore_patterns:
                    if any(re.search(pattern, str(value), re.IGNORECASE) for value in row):
                        should_ignore = True
                        break
    ...
    def validate_table(self, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            # 필수 컬럼 체크
            required_columns = ['담보명', '지급사유', '지급금액']
